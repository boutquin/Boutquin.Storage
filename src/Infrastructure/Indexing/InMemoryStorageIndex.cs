...
/// <remarks>
/// <para>
/// <b>Why SortedDictionary for the index?</b> The index needs O(log n) lookup by key and the ability
/// to iterate in sorted order (for range scans and SSTable flushing). <see cref="SortedDictionary{TKey, TValue}"/>
/// provides both. A HashMap would give O(1) lookups but no ordering; a <see cref="SortedList{TKey, TValue}"/>
/// would give O(log n) lookups but O(n) inserts. SortedDictionary is the best balance for index
/// workloads where inserts and lookups are equally frequent.
/// </para>
/// <para>
