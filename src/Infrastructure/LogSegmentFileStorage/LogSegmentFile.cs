...
/// <remarks>
/// <para>
/// <b>Why a decorator around IFileBasedStorageEngine?</b> LogSegmentFile adds size-limit enforcement
/// (throwing on overflow) without reimplementing storage logic. This is the Decorator pattern — it
/// wraps any <see cref="IFileBasedStorageEngine{TKey, TValue}"/> and adds the segment-size constraint.
/// The underlying engine handles serialization, file I/O, and compaction; this class only enforces the
/// size boundary.
/// </para>
/// <para>
/// <b>Why throw on SetAsync when full, rather than silently creating a new segment?</b> Segment
/// lifecycle management belongs to <see cref="LogSegmentedStorageEngine{TKey, TValue}"/> (the
/// orchestrator), not to individual segments. A segment that silently creates successors would violate
/// the single-responsibility principle and make segment tracking impossible. Throwing forces the caller
/// to handle the overflow, keeping segment creation centralized.
/// </para>
/// </remarks>
    ...
    where TValue : ISerializable<TValue>, new()
{
    ...
    {
        // Why check before write rather than after? This is a soft limit — the check prevents
        // starting new writes on a full segment, but the current write may push the segment
        // slightly over _maxSegmentSize. Exact enforcement would require pre-calculating entry
        // size, which is expensive. The orchestrator (LogSegmentedStorageEngine) handles the
        // overage tolerance by checking size before each write.
        if (_storageEngine.FileSize >= _maxSegmentSize)
        {
            throw new InvalidOperationException("Segment size exceeded.");
        }
        await _storageEngine.SetAsync(key, value, cancellationToken).ConfigureAwait(false);
    }
