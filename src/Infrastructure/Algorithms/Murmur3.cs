...
{
    private const uint Seed = 0xc58f1a7b; // Seed value for the Murmur3 hash
    ...
    {
        uint hash = Seed; // Initialize hash with the seed value
        int length = data.Length; // Get the length of the input data
        int remainder = length & 3; // Calculate the remainder of the data length divided by 4
        int blocks = length >> 2; // Calculate the number of 4-byte blocks
        ...
        unsafe
        {
            fixed (byte* ptr = data)
            {
                ...
                {
                    uint k = blocksPtr[i]; // Get the current block
