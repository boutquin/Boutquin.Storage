...
/// <remarks>
/// <para>
/// <b>Why Murmur3?</b> Chosen for Bloom filter hashing because it has excellent avalanche
/// properties (every input bit affects ~50% of output bits) and is significantly faster than
/// cryptographic hashes. Not suitable for security — only for hash-table-style lookups where
/// uniform distribution matters more than collision resistance.
/// </para>
/// <para>
/// <b>Why this seed value (0xc58f1a7b)?</b> The seed is an arbitrary constant; any non-zero value
/// works. This particular value was chosen for reproducibility — changing it would produce different
/// hash outputs, breaking existing Bloom filter state.
/// </para>
/// <para>
/// <b>Why the specific magic constants (0xcc9e2d51, 0x1b873593)?</b> These are part of Austin
/// Appleby's original Murmur3 specification. They were selected through empirical testing to
/// maximize bit mixing (avalanche effect). They are not arbitrary — replacing them degrades hash
/// quality.
/// </para>
/// <para>
/// <b>Why process in 4-byte blocks?</b> Murmur3 is designed to process 32-bit words at a time for
/// performance on 32-bit architectures. The remainder handling ensures inputs not aligned to 4 bytes
...
public class Murmur3 : IHashAlgorithm
{
    ...
    /// <remarks>
    /// <para>
    /// <b>Why AggressiveInlining?</b> This is a hot-path helper method called per block. Inlining
    /// eliminates call overhead, which matters when hashing millions of keys in a Bloom filter.
    /// </para>
    /// </remarks>
    ...
    /// <remarks>
    /// <para>
    /// <b>Why the FMix finalization step?</b> Without finalization, the lower bits of the hash are
    /// biased by the last input block. FMix applies additional mixing (shift-multiply-xor) to ensure
    /// all 32 output bits are influenced by all input bits — critical for Bloom filter bit-position
    /// uniformity.
    /// </para>
    /// <para>
    /// <b>Why AggressiveInlining?</b> Called once per hash computation on the hot path. Inlining
    /// eliminates call overhead when hashing millions of keys in a Bloom filter.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
