...
/// <remarks>
/// <para>
/// <b>Why SortedDictionary instead of Dictionary?</b> <see cref="SortedDictionary{TKey, TValue}"/>
/// maintains keys in sorted order (via the red-black tree it uses internally), which is essential
/// for LSM-tree MemTable semantics. When the MemTable is flushed to an SSTable, entries must be
/// written in sorted key order. A regular <see cref="Dictionary{TKey, TValue}"/> would require a
/// separate sort step before flushing, adding O(n log n) overhead at flush time.
/// </para>
/// <para>
/// <b>Why does RemoveAsync throw NotSupportedException?</b> This mirrors the append-only semantics
/// of the storage engine architecture. In an LSM-tree, deletes are represented as tombstone entries
/// (a special marker value), not as actual removals from the data structure. The caller should write
/// a tombstone via <see cref="SetAsync"/> rather than removing the key.
/// </para>
/// <para>
/// <b>Why synchronous operations behind an async interface?</b> The <see cref="IBulkStorageEngine{TKey, TValue}"/>
/// interface is async to accommodate implementations backed by disk I/O or network calls. This
/// in-memory implementation completes instantly, so it returns pre-completed tasks
/// (<see cref="Task.CompletedTask"/> / <see cref="Task.FromResult{TResult}"/>) to avoid async
/// state machine overhead while honoring the interface contract.
/// </para>
/// </remarks>
