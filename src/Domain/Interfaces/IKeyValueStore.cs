...
///
/// <para><b>Interface Hierarchy:</b></para>
/// <para>The storage interface family splits along two axes: serialization requirements and
/// bulk operation support. The left branch requires only <c>IComparable</c> (suitable for
/// in-memory data structures). The right branch adds <c>ISerializable</c> constraints
/// (required for on-disk persistence).</para>
/// <code>
/// IReadOnlyKeyValueStore{TKey, TValue}         TKey : IComparable
///   └─ IKeyValueStore{TKey, TValue}            + Set, Remove, Clear
///        │
///        ├─ IStorageEngine{TKey, TValue}        TKey : ISerializable (no new methods — marker for serializable stores)
///        │
///        └─ IBulkKeyValueStore{TKey, TValue}    + GetAllItems, SetBulk (TKey : IComparable only)
///             │
///             ├─ IBTree, IBPlusTree, IMemTable   In-memory data structures (no ISerializable needed)
///             │
///             └─ IBulkStorageEngine{TKey, TValue}   Combines IStorageEngine + IBulkKeyValueStore (TKey : ISerializable)
///                  │
