...
///
/// <para><b>Complexity (where n = number of entries):</b></para>
/// <para>- <b>SetAsync (insert/update):</b> O(log n) — tree traversal + at most 2 rotations.</para>
/// <para>- <b>TryGetValueAsync / ContainsKeyAsync:</b> O(log n) — binary search down the tree.</para>
/// <para>- <b>RemoveAsync:</b> Not supported (LSM-tree uses tombstones instead of deletion).</para>
/// <para>- <b>GetAllItemsAsync:</b> O(n) — in-order traversal of all nodes.</para>
/// <para>- <b>SetBulkAsync:</b> O(k log n) where k = number of items inserted.</para>
/// <para>- <b>ClearAsync:</b> O(1) — nulls the root reference.</para>
...
/// section on LSM-trees and memtables. The memtable is an in-memory balanced tree (e.g., red-black tree) that buffers
/// writes before flushing to sorted on-disk segments (SSTables).</para>
/// </remarks>
