...
public sealed class RedBlackTreeTests
{
    ...
    public async Task Add_ShouldThrowExceptionIfTreeIsFull()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => tree.SetAsync(2, "value2"));
    }
    ...
    /// <summary>
    /// Test to ensure that GetAllItemsAsync works correctly with a large number of items,
    /// verifying that the iterative in-order traversal doesn't cause stack overflow.
    /// </summary>
    [Fact]
    public async Task GetAllItems_ShouldHandleLargeTree_WithoutStackOverflow()
    {
        // Arrange: Create a tree large enough that recursive traversal could be risky.
        const int count = 10_000;
        var tree = new RedBlackTree<int, string>(count);

        // Insert in a pattern that creates deep trees (sequential insert)
        ...
        // Assert: All items present and in sorted order
        Assert.Equal(count, items.Count);
        for (var i = 0; i < count; i++)
        {
            Assert.Equal(i, items[i].Key);
            Assert.Equal($"value_{i}", items[i].Value);
        }
    }
    ...
    /// <summary>
    /// Test that SetAsync throws ArgumentNullException when a null key is provided (M25-M26).
    /// Uses string keys (reference type) since null is only applicable to reference types.
    /// </summary>
    [Fact]
    public async Task SetAsync_WithNullKey_ShouldThrowArgumentNullException()
    {
        // Arrange
        var tree = new RedBlackTree<string, string>(10);
